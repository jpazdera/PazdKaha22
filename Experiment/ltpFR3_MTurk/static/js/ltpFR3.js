...
var audio_test_intro = {
	type: 'text',
	text: '<p id="inst">Due to the use of auditory stimuli in this task, it is important that you have the volume on your computer turned on and set loud enough to hear the words that we will be presenting. In order to ensure that you will be able to perform the auditory portions of the task, we ask that you begin by completing the following audio test.</p><p id="inst">You will see a series of three pages, each containing a play button and an empty textbox. On each page, click the play button to listen to an audio clip of a single word, then enter that word into the textbox to proceed. You will be able to replay the word as many times as needed, giving you the chance to adjust your volume to an appropriate level. Press any key to begin the audio test.</p>',
	data: {type: 'INSTRUCTIONS'}
};
...
var instructions = {
  ...
  text: '<p id="inst">During the course of this study, you will see and hear lists of words, which you will be asked to remember. Some sets of words will be presented visually on-screen, one at a time. Other sets of words will be presented as a sequence of audio clips. As you see or hear each word, try to focus on that word alone, without thinking back to previous words. After each series of words ends, a row of asterisks will flash on the screen, and you will see a screen with an empty textbox appear.<p id="inst">When the textbox appears, try to recall as many words as possible from the most recent list. As you recall each word, type it into the provided textbox. Pressing ENTER, SPACE, COMMA, or SEMICOLON will submit the word and clear the textbox. Please type words into the box one at a time, submitting each as you go.</p><p id="inst">You do not need to worry about capitalization while entering your words. If you are unsure of how to spell a word, simply type it to the best of your ability. You will be given a fixed amount of time to recall the words. Please try hard throughout the recall period, as you may recall some words even when you feel that you have exhausted your memory.</p><p id="inst">Press any key to continue reading the instructions...</p>',
  data: {type: 'INSTRUCTIONS'}
};
...
var instructions_practice = {
  type: 'text',
  text: '<p id="inst">You will now have the chance to familiarize yourself with the task by completing two practice trials. Remember that there are three steps in each trial. First, you will be asked to complete a series of math problems of the form A+B+C. Then, you will either see or hear a series of words, presented one at a time. Finally, you will be asked to recall the words in any order, entering each into the provided textbox.</p><p id="inst">Press any key to begin the practice trials.</p>',
...
var instructions_end_practice = {
  ...
  data: {type: 'INSTRUCTIONS'},
  on_finish: function() {
    psiturk.finishInstructions();  // Change status from 1 to 2 in database
  }
};
